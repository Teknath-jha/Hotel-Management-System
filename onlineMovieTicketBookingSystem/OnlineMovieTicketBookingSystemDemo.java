...
import onlineMovieTicketBookingSystem.Seat.SeatType;
import onlineMovieTicketBookingSystem.booking.Booking;

public class OnlineMovieTicketBookingSystemDemo{
    public static void main(String[] args) {
        OnlineMovieTicketBookingSystem bookingSystem = OnlineMovieTicketBookingSystem.getInstance();
    ...
    private static Map<String, Seat> createSeats(int rows, int columns) {
        ...
        for (int row = 1; row <= rows; row++) {
            for (int col = 1; col <= columns; col++) {
                ...
                Seat seat = new Seat(seatId, seatType,  SeatStatus.AVAILABLE, row, col, price);
